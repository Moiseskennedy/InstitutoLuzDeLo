...
// 1. Nome: Não pode ter números nem caracteres especiais
document.getElementById('nome').addEventListener('input', function() {
    const nomeInput = this;
    const nomeValue = nomeInput.value;
    // Regex: ^[A-Za-zÀ-ÖØ-öø-ÿ\s]+$
    // ^      : Início da string
    // [A-Za-z: Permite letras de a-z e A-Z (sem acento)
    // À-ÖØ-öø-ÿ: Permite letras acentuadas comuns em português e outras línguas
    // \s     : Permite espaços em branco
    // +      : Pelo menos um caractere
    // $      : Fim da string
    // Ou seja, a string deve conter APENAS letras (acentuadas ou não) e espaços.
    const regex = /^[A-Za-zÀ-ÖØ-öø-ÿ\s]+$/;

    if (!nomeValue.trim()) { // Verifica se está vazio ou só com espaços
        displayError('nomeError', 'O campo Nome é obrigatório.');
        nomeInput.setCustomValidity('O campo Nome é obrigatório.');
    } else if (!regex.test(nomeValue)) {
        displayError('nomeError', 'O nome não pode conter números ou caracteres especiais.');
        nomeInput.setCustomValidity('O nome não pode conter números ou caracteres especiais.');
    } else {
...
document.getElementById('cadastroForm').addEventListener('submit', function(event) {
    ...
    document.getElementById('nome').dispatchEvent(new Event('input')); // Mudei para 'input' para o nome, é mais consistente
